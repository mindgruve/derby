...
class ImageGDTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUpImages($sourceKey, $targetKey, $secondaryTargetKey = null, $dispatcher = null)
    {
        ...
        if ($secondaryTargetKey) {
            ...
            $this->secondaryTargetFile = new \Derby\Media\File\Image(
                $secondaryTargetKey,
                $secondaryTargetAdapter,
                $imagine,
                $dispatcher
            );
...
//            $this->assertLessThan(
//                1,
//                abs(floor($sourceDimensions[1] / $sourceDimensions[0] * 100)) - $imageDimensions[1]
//            );
//
//
//            /**
...
//            $this->assertLessThan(
//                1,
//                abs(floor($sourceDimensions[0] / $sourceDimensions[1] * 100)) - $imageDimensions[0]
//            );
//        }
//    }
//
...
//    }
//
    public function testConvertFocusPointX()
    {
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-vNORTH-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);

        $this->sourceFile->setFocusPointX(-1);
        $this->assertEquals(0, $this->sourceFile->convertFocusPointX($this->sourceFile->getWidth()));

        $this->sourceFile->setFocusPointX(-.5);
        $this->assertEquals(200, $this->sourceFile->convertFocusPointX($this->sourceFile->getWidth()));

        $this->sourceFile->setFocusPointX(0);
        $this->assertEquals(400, $this->sourceFile->convertFocusPointX($this->sourceFile->getWidth()));

        $this->sourceFile->setFocusPointX(.5);
        $this->assertEquals(600, $this->sourceFile->convertFocusPointX($this->sourceFile->getWidth()));

        $this->sourceFile->setFocusPointX(1);
        $this->assertEquals(800, $this->sourceFile->convertFocusPointX($this->sourceFile->getWidth()));
    }

    public function testGDConvertFocusPointY()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $this->sourceFile->setFocusPointY(-1);
        $this->assertEquals(866, $this->sourceFile->convertFocusPointY($this->sourceFile->getHeight()));

        $this->sourceFile->setFocusPointY(-.5);
        $this->assertEquals(649.5, $this->sourceFile->convertFocusPointY($this->sourceFile->getHeight()));

        $this->sourceFile->setFocusPointY(0);
        $this->assertEquals(433, $this->sourceFile->convertFocusPointY($this->sourceFile->getHeight()));

        $this->sourceFile->setFocusPointY(.5);
        $this->assertEquals(216.5, $this->sourceFile->convertFocusPointY($this->sourceFile->getHeight()));

        $this->sourceFile->setFocusPointY(1);
        $this->assertEquals(0, $this->sourceFile->convertFocusPointY($this->sourceFile->getHeight()));
    }


    public function testGdResizeWithFocusPoint400x100Center()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x100-vCenter-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);

        $this->sourceFile->setFocusPointX(0);
        $this->sourceFile->setFocusPointY(0);
        $this->sourceFile->resize(400, 100, Image::THUMBNAIL_OUTBOUND)->save($this->targetFile);
    }


    public function testGdResizeWithFocusPoint400x100North()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x100-vNORTH-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint400x100South()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x100-vSOUTH-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint400x100East()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x100-vEAST-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint400x100WEST()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x100-vWEST-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint100x400Center()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-100x400-vCenter-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);

        $this->sourceFile->setFocusPointX(0);
        $this->sourceFile->setFocusPointY(0);
        $this->sourceFile->resize(100, 400, Image::THUMBNAIL_OUTBOUND)->save($this->targetFile);
    }


    public function testGdResizeWithFocusPoint100x400North()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-100x400-vNORTH-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint100x400South()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-100x400-vSOUTH-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint100x400East()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-100x400-vEAST-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint100x400WEST()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-100x400-vWEST-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint400x400Center()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x400-vCenter-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);

        $this->sourceFile->setFocusPointX(0);
        $this->sourceFile->setFocusPointY(0);
        $this->sourceFile->resize(400, 400, Image::THUMBNAIL_OUTBOUND)->save($this->targetFile);
    }


    public function testGdResizeWithFocusPoint400x400North()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x400-vNORTH-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint400x400South()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x400-vSOUTH-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint400x400East()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x400-vEAST-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
    ...
    public function testGdResizeWithFocusPoint400x400WEST()
    {
        if (!extension_loaded('gd')) {
            return;
        }
        ...
        $sourceKey = 'test-800x866.jpg';
        $targetKey = 'gd-resize-with-focuspoint-400x400-vWEST-'.$sourceKey;
        $this->setUpImages($sourceKey, $targetKey);
